# `architecture.py` · 建筑与地标骨架

对应已新增文件：[scripts/architecture.py](../../scripts/architecture.py)。本轮只提供初版框架，未运行Blender验收。

## 这个Python文件利用什么，实现什么

它使用纯Python参数化开间、柱梁和屋顶。主要入口是`hall(...) / build(builder, config, plan)`。输入layout的尺寸、楼层、旋转与类型；输出民居/商铺/客栈/仓房体块与12处地标或保留台。

实现方法：hall按墙片围出门洞，柱列和屋面独立；地标组合正房、两翼与门厅；望楼为三个逐级收分体量；D07只建保留平台。
...
普通家族目前共享hall拓扑，未细化真实门窗、楼梯、室内、瓦片；三层望楼也只是外部体量，不能声称可登楼。
